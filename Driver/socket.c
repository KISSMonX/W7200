...
{
        if(netinfo->Mac[0] != 0x00 || netinfo->Mac[1] != 0x00
           || netinfo->Mac[2] != 0x00 || netinfo->Mac[3] != 0x00
           || netinfo->Mac[4] != 0x00 || netinfo->Mac[5] != 0x00) {
                setSHAR(netinfo->Mac);        // set local MAC address
        }

        if(netinfo->IP[0] != 0x00 || netinfo->IP[1] != 0x00
           || netinfo->IP[2] != 0x00 || netinfo->IP[3] != 0x00) {
                setSIPR(netinfo->IP);        // set local IP address
        }

        if(netinfo->Subnet[0] != 0x00 || netinfo->Subnet[1] != 0x00
           || netinfo->Subnet[2] != 0x00 || netinfo->Subnet[3] != 0x00) {
        ...
        if(netinfo->Gateway[0] != 0x00 || netinfo->Gateway[1] != 0x00
           || netinfo->Gateway[2] != 0x00 || netinfo->Gateway[3] != 0x00) {
                setGAR(netinfo->Gateway);        // set gateway address
        }

        if(netinfo->DNSServerIP[0] != 0x00 || netinfo->DNSServerIP[1] != 0x00
           || netinfo->DNSServerIP[2] != 0x00 || netinfo->DNSServerIP[3] != 0x00) {
...
int8 GetTCPSocketStatus(SOCKET s)
{
        ...
        case SOCK_LAST_ACK:		/**< closing state */
                ...
                break;
        default:
                if((IINCHIP_READ(Sn_MR(s)) & 0x0F) != Sn_MR_TCP)	{
                        ret = (int8)ERROR_NOT_TCP_SOCKET;
                }
                break;
        }
...
int8 TCPClientOpen(SOCKET s, uint16 port, uint8 * destip, uint16 destport)
{
        ...
        if ( ((destip[0] == 0xFF) && (destip[1] == 0xFF) && (destip[2] == 0xFF) && (destip[3] == 0xFF))
...
int16 TCPSend(SOCKET s, const uint8 * src, uint16 len)
{
        ...
        if((IINCHIP_READ(Sn_MR(s)) & 0x0F) != Sn_MR_TCP)	{
                return ERROR_NOT_TCP_SOCKET;
        }

        if(status == SOCK_FIN_WAIT) {
                return ERROR_FIN_WAIT;
        }

        if(status != SOCK_ESTABLISHED && status != SOCK_CLOSE_WAIT)	{
                return ERROR_NOT_ESTABLISHED;
        }
...
int16 TCPRecv(SOCKET s, uint8 * buf, uint16 len)
{
        ...
        if (RSR_len == 0) {
                ...
                if((IINCHIP_READ(Sn_MR(s)) & 0x0F) != 0x01) {
                        return ERROR_NOT_TCP_SOCKET;
                }

                if(status == SOCK_CLOSE_WAIT) {
                   return ERROR_CLOSE_WAIT;
                }

                if(status != SOCK_ESTABLISHED && status != SOCK_FIN_WAIT)	{
                        return ERROR_NOT_ESTABLISHED;
                }

                if(RSR_len == 0) {
                        return  RSR_len;        // Check Receive Buffer of W5200
                }
        }
...
int16 UDPRecv(SOCKET s, uint8 * buf, uint16 len, uint8 * addr, uint16 *port)
{
        ...
        if((RSR_len = GetSocketRxRecvBufferSize(s)) == 0)	{
                return RSR_len;        // Check Receive Buffer of W5200
        }
